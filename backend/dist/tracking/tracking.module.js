...
let TrackingModule = TrackingModule_1 = class TrackingModule {
    static forRoot(options, database, passport) {
        return {
            module: TrackingModule_1,
            imports: [nest_shopify_1.ShopifyModule.forRoot(options, database, passport)],
        };
    }
    configure(consumer) {
        consumer
            .apply(nest_shopify_1.BodyParserJsonMiddleware)
            .forRoutes(settings_controller_1.SettingsController);
    }
};
TrackingModule = TrackingModule_1 = __decorate([
    common_1.Module({
        providers: [
