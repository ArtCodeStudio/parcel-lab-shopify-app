...
export class ParcelLabApi {
    ...
    if (utils.objHasKeys(payload, requiredKeys)) {
      if (utils.objHasOnlyKeys(payload, allowedKeys)) {
        ...
        for (let i = 0; i < datachecks.email.length; i++) {
          if (
            !_.isNull(payload[datachecks.email[i]]) &&
            !_.isUndefined(payload[datachecks.email[i]])
          ) {
        ...
        for (let j = 0; j < datachecks.number.length; j++) {
          if (
            !_.isNull(payload[datachecks.number[j]]) &&
            !_.isUndefined(payload[datachecks.number[j]])
          ) {
            isValid =
        ...
        for (let k = 0; k < datachecks.boolean.length; k++) {
          if (
            !_.isNull(payload[datachecks.boolean[k]]) &&
            !_.isUndefined(payload[datachecks.boolean[k]])
          ) {
            isValid =
  ...
  protected multiplyOnTrackingNumber(
    ...
    if (terminator === 'json') {
      ...
      for (let k = 0; k < jsonCouriers.length; k++) {
        const courier = this.guessCourier(
          jsonCouriers[k],
          payload.destination_country_iso3,
        );
  ...
  protected guessCourier(
    ...
    if (
      params.couriersAppendCountry.includes(output) &&
      destinationCountryIso3
    ) {
      console.warn(
        'Append country code to courier, please check if this is the correct courier you use.',
      );
